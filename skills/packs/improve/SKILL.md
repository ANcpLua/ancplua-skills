...
license: MIT
metadata:
  author: shadcn (Fable 5 adaptation by Alex)
  version: "1.2.0-fable"
...
The economics of this skill: the advisor does the part where intelligence compounds (understanding, judging, specifying) and holds the whole picture; workers do the volume in their own context windows. The plan is the product — its quality determines whether the executor succeeds.

**Model policy — Fable 5 advisor, Opus 5 workers.** Aligned with the standing `multi-agent-teams` policy; do not diverge from it here.

YOU — the advisor: recon, vetting, prioritization, plan writing, diff review — run on the session model, **Fable 5 at 1M context**. Never delegate judgment. Delegate *volume*, not thinking: audit-sweep subagents and `execute` executors both run on **Opus 5** (`model: "opus"`).

That worker tier is deliberate, and it is a change from this skill's upstream economics. Phase 3 of this very workflow exists because sweep subagents over-report, mis-attribute evidence to the wrong `file:line`, and flag by-design behavior as findings. A cheaper sweeper does not save money when the advisor has to re-open and re-verify every citation — it relocates the cost into your context and adds a failure class (a wrong excerpt becomes a wrong plan that fails its own drift check). The same holds for executors: a plan is only worth dispatching if the executor can actually follow it.

**Depth scales coverage and concurrency, not model tier.** All subagents are Opus 5 at every effort level.
...
For repos of any real size, fan out with parallel read-only subagents (in Claude Code: **Explore** agents) — one per category (or cluster of related categories). At `deep` level, when the **Workflow tool** is available AND the user has opted into multi-agent orchestration, prefer it over ad-hoc Agent calls: pipeline `category finder → adversarial verifier` per category with structured-output schemas, so mis-attributed findings die before they reach your vetting pass (Phase 3 still applies — the workflow reduces noise, it doesn't replace your own reads). If the host agent can't spawn subagents, audit directly yourself in category-priority order. **Subagents do not inherit this skill's context**, so each subagent prompt must include:
...
- `execute <plan>` → dispatch an executor subagent on one plan (Agent tool with `isolation: "worktree"` and `model: "opus"`), then review its diff like a tech lead — re-run done criteria, check scope, read the code — and render a verdict. Treat the executor's diff as untrusted until reviewed: verify every hunk traces to a plan step and reject any out-of-scope change, however plausible it looks. Requires a host agent that can spawn subagents in an isolated worktree; if yours can't, say so and hand the plan over for manual execution instead. **Read [references/closing-the-loop.md](references/closing-the-loop.md) before the first dispatch.**
