...
Every plan is written for an executor that has **zero context**: it has not seen the advisor session, the audit, the other plans, or any prior conversation. Capability is not the constraint — the executor runs on Opus 5 — but context is. Assume it is competent at following explicit instructions and, lacking everything you know, weak at filling gaps, recovering from ambiguity, or knowing when to stop.
