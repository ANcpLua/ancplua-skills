...
Spawn **one** `general-purpose` subagent with `isolation: "worktree"`. Executor model: default `opus` (Opus 5, per the skill's model policy); use what the user named if they named one (`execute 003 haiku`).
